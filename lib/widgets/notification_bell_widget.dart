...
class _NotifTile extends StatelessWidget {
  ...
  // ── Types ─────────────────────────────────────
  bool get _isLivree => notif.type == 'commande_livree';
  bool get _isDemande => notif.type == 'nouvelle_demande';
  bool get _isApprouvee => notif.type == 'demande_approuvée';
  bool get _isRefusee => notif.type == 'demande_refusée';
  bool get _isCommission =>
      notif.type == 'commission_vendeur' || notif.type == 'commission_admin';

  // ── Couleur selon type ────────────────────────
  Color get _color {
    if (_isLivree) return const Color(0xFF059669);
    if (_isDemande) return const Color(0xFF7C3AED);
    if (_isApprouvee) return const Color(0xFF059669);
    if (_isRefusee) return const Color(0xFFDC2626);
    if (_isCommission) return const Color(0xFFFFD700);
    return AppColors.primary;
  }

  // ── Icône liste selon type ────────────────────
  IconData get _icon {
    if (_isLivree) return Icons.local_shipping_outlined;
    if (_isDemande) return Icons.storefront_outlined;
    if (_isApprouvee) return Icons.check_circle_outline;
    if (_isRefusee) return Icons.cancel_outlined;
    if (_isCommission) return Icons.account_balance_wallet_outlined;
    return Icons.notifications_outlined;
  }

  // ── Icône dialog selon type ───────────────────
  IconData get _iconDetail {
    if (_isLivree) return Icons.local_shipping_rounded;
    if (_isDemande) return Icons.storefront_rounded;
    if (_isApprouvee) return Icons.check_circle_rounded;
    if (_isRefusee) return Icons.cancel_rounded;
    if (_isCommission) return Icons.account_balance_wallet_rounded;
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () async {
        ...
        if (context.mounted) _showNotifDetail(context);
      },
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 200),
        color:
