...
// // KPI cards + graphiques barres + camembert
// // ─────────────────────────────────────────────
...
//       // Barres revenus
//       _revenusParMois = revenusMap.entries
...
//       _categoriesProduits = sorted.take(6).toList().asMap().entries
//           .map((e) => _CatData(
//                 nom:    e.value.key,
//                 count:  e.value.value,
...
//           .get();
//       final pro  = snap.docs
//           .where((d) => d.data()['type'] == 'professionnel')
//           .length;
...
//                 fontWeight: FontWeight.w900, color: _textDark)),
...
//     final maxRevenu = _revenusParMois.isEmpty
//         ? 1.0
//         : _revenusParMois
//             .map((e) => e.revenu)
//             .reduce((a, b) => a > b ? a : b)
...
//               children: _revenusParMois.map((m) {
//                 final ratio = maxRevenu > 0
//                     ? (m.revenu / maxRevenu)
//                     : 0.0;
//                 final isMax = m.revenu == maxRevenu &&
//                     m.revenu > 0;
//                 return Expanded(
//                   child: Padding(
//                     padding: const EdgeInsets.symmetric(
...
//     final total = _categoriesProduits.isEmpty
//         ? 1
//         : _categoriesProduits
//             .map((e) => e.count)
//             .reduce((a, b) => a + b);
...
//             ),
//             const SizedBox(width: 10),
//             const Text('Top 5 produits commandés',
//                 style: TextStyle(fontSize: 15,
...
//     final statuts = [
//       _StatutData('En attente', enAttente,  _orange,
//           Icons.access_time_rounded),
//       _StatutData('Livrées',    livrees,    _green,
//           Icons.done_all_rounded),
//       _StatutData('Annulées',   annulees.clamp(0, 99999), _red,
...
//   const _KpiData({
//     required this.titre, required this.valeur,
//     required this.sous, required this.icon,
...
//   final Color color;
//   const _CatData({
//     required this.nom, required this.count, required this.color});
// }
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _computePredictions() async {
    try {
      ...
      final commandesSnap = await FirebaseFirestore.instance
          .collection('commandes')
          .get();
      final produitsSnap = await FirebaseFirestore.instance
          .collection('produits')
          .get();
      ...
      if (mounted) {
        setState(() {
          ...
          _predictionRevenu = MLPredictionService.predictRevenusNextMonth(revenusHistory);

          // Prédiction produits
          final allProductNames = produitsList
        ...
        children: [
          Row(children: [
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Prédiction Revenus',
                    style: TextStyle(fontSize: 14,
                        fontWeight: FontWeight.w800, color: _textDark)),
                Text('Mois prochain (confiance: ${pred.confidence.toStringAsFixed(0)}%)',
                    style: const TextStyle(
                        fontSize: 11, color: _textMuted)),
              ],
            ),
          ...
          const SizedBox(height: 16),
          Text(_formatMontant(pred.prediction),
      ...
      child: Column(
        ...
        children: [
          ...
          if (_produitsPredits.isEmpty)
            const Text('Données insuffisantes',
                style: TextStyle(color: _textMuted, fontSize: 12))
          else
            ..._produitsPredits.take(3).map((p) => Padding(
                  padding: const EdgeInsets.only(bottom: 12),
                  child: Row(
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(p.nom,
                                ...
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis),
                            Text('Score: ${p.potentiel.toStringAsFixed(1)}',
                                style: const TextStyle(
        ...
        children: [
          Row(children: [
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Segmentation Clients',
                    style: TextStyle(fontSize: 14,
                        fontWeight: FontWeight.w800, color: _textDark)),
                Text(
  ...
  Widget _buildRisquesStockCard() {
    final critiques = _predictionsStock
        .where((p) => p.riskLevel == 'critique')
        .toList();
    final eleves = _predictionsStock
        .where((p) => p.riskLevel == 'élevé')
        .toList();
