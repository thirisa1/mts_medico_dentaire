...
import '../../services/client_prediction_service.dart';
...
class _AcheteurHomeScreenState extends State<AcheteurHomeScreen> {
  ...
  List<PricePredictor> _pricePredictions = [];
  List<FavoriAlert> _favoriAlerts = [];
  ...
  Future<void> _loadMLPredictions() async {
    try {
      ...
      final purchases = purchasesSnap.docs.map((d) => d.data()).toList();
      final favoris = favoriSnap.docs.map((d) => d.data()).toList();
      final products = productsSnap.docs.map((d) => d.data()).toList();

      if (mounted) {
        setState(() {
          _clientProfile = ClientPredictionService.createClientProfile(
            user.uid,
            purchases,
          );
          _recommendations = ClientPredictionService.recommendProducts(
            purchases,
            products,
          );
          _pricePredictions = ClientPredictionService.predictPrices(
            products,
            purchases,
          );
          _favoriAlerts = ClientPredictionService.generateFavoriAlerts(
            favoris,
            products,
          );
      ...
      debugPrint('Erreur ML predictions: $e');
      if (mounted) {
  ...
  Widget _buildDashboard() {
    ...
    return Column(
      ...
      children: [
        ...
        // Alertes favoris
        if (_favoriAlerts.isNotEmpty) ...[
          _buildSectionTitle('Alertes favoris'),
          const SizedBox(height: 16),
          _buildFavoriAlerts(),
          const SizedBox(height: 32),
        ],
        ...
        if (_recommendations.isNotEmpty) ...[
          _buildSectionTitle('Recommandé pour vous'),
          const SizedBox(height: 16),
        ...
        // Prédictions prix
        if (_pricePredictions.isNotEmpty) ...[
          _buildSectionTitle('Tendance des prix'),
          const SizedBox(height: 16),
          _buildPricePredictions(),
        ],
      ],
    );
  }
  ...
  Widget _buildClientStats() {
    if (_clientProfile == null) return const SizedBox();
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildStatCard(
                  'Par mois',
                  '${profile.frequenceAchat.toStringAsFixed(1)}',
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${profile.depenseMoyenne.toStringAsFixed(0)} DA',
                      style: const TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.w800,
                        color: AppColors.primary,
  ...
  // ── Alertes favoris ──
  Widget _buildFavoriAlerts() {
    return ListView.builder(
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      itemCount: _favoriAlerts.length,
      itemBuilder: (context, i) {
        final alert = _favoriAlerts[i];
        final colorMap = {
          'en_stock': const Color(0xFF059669),
          'rupture_prevue': const Color(0xFFDC2626),
          'new_variant': const Color(0xFFF59E0B),
        };
        final color = colorMap[alert.etat] ?? AppColors.primary;
        ...
        return Container(
          ...
          child: Row(
            children: [
              Text(alert.icone, style: const TextStyle(fontSize: 24)),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    LinearProgressIndicator(
                      value: rec.scoreRecommandation / 100,
  ...
  // ── Tendance prix ──
  Widget _buildPricePredictions() {
    return ListView.builder(
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      itemCount: _pricePredictions.length,
      itemBuilder: (context, i) {
        final pred = _pricePredictions[i];
        final colorTendance =
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    '${pred.prixActuel.toStringAsFixed(0)} DA',
                    style: const TextStyle(
                      fontSize: 12,
                      color: AppColors.textMuted,
                    ),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: colorTendance.withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Row(
                  children: [
                    Icon(iconTendance, color: colorTendance, size: 18),
                    const SizedBox(width: 6),
                    Text(
                      '${pred.pourcentageChangement > 0 ? '+' : ''}${pred.pourcentageChangement.toStringAsFixed(1)}%',
                      style: TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.w700,
                        color: colorTendance,
  ...
  //             final solde = (data?['soldeGains'] ?? 0.0).toDouble();
  //             final total = (data?['totalGagné'] ?? 0.0).toDouble();
  ...
  //                   const SizedBox(height: 4),
  //                   Text(
  //                     'Total cumulé : ${total.toStringAsFixed(0)} DA',
  //                     style: const TextStyle(
  //                       color: Colors.white70,
  //                       fontSize: 12,
  //                     ),
  ...
  //                     const SizedBox(height: 6),
  //                     const Text(
  //                       'Vos gains apparaîtront ici après chaque vente.',
  //                       style: TextStyle(
  //                         fontSize: 12,
  //                         color: AppColors.textMuted,
  //                       ),
  ...
  //                   ...docs.map((doc) {
  //                     final d = doc.data() as Map<String, dynamic>;
  //                     final partV = (d['partVendeur'] ?? 0).toDouble();
  //                     final totalV = (d['totalVente'] ?? 0).toDouble();
  //                     final nom = d['productNom'] ?? '—';
  //                     final ref = d['orderRef'] ?? '';
  ...
  //                               ),
  //                               Text(
  //                                 'sur ${totalV.toStringAsFixed(0)} DA',
  //                                 style: const TextStyle(
  //                                   fontSize: 10,
  //                                   color: AppColors.textMuted,
  //                                 ),
  ...
  Widget _buildEmptyState(
    ...
    return Container(
      ...
      child: Column(
        children: [
          Icon(icon, size: 56, color: AppColors.primary.withValues(alpha: 0.25)),
          const SizedBox(height: 14),
          Text(
            message,
            textAlign: TextAlign.center,
